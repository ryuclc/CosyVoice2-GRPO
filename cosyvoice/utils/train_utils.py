...
def init_optimizer_and_scheduler(args, configs, model, gan):
    if gan is False:
        ...
        elif configs['train_conf']['scheduler'] == 'cosinelr':
...
def batch_forward(model, batch, scaler, info_dict, ref_model=None, dpo_loss=None, grpo_flow_model=None, grpo_hift_model=None):
    ...
    with autocast:
        if ref_model is not None and grpo_flow_model is not None and grpo_hift_model is not None:
            info_dict['loss_dict'] = model(batch, device, ref_model, grpo_flow_model, grpo_hift_model)

            if wandb.run is not None:
                mode = "train" if model.training else "eval"
                wandb_metrics = {}
                for k, v in info_dict['loss_dict']['_metrics'].items():
                    wandb_metrics[f"{mode}/{k}"] = v

                wandb.log(wandb_metrics)
            
            del info_dict['loss_dict']['_metrics']
        else:
        ...
        if ref_model is not None and dpo_loss is not None:
            ...
            info_dict['loss_dict']["chosen_logps"] = chosen_logps.mean()
            info_dict['loss_dict']["rejected_logps"] = rejected_logps.mean()
