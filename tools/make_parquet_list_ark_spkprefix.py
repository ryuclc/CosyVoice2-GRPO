...
def job(utt_list, parquet_file, utt2parquet_file, spk2parquet_file, spk_prefix):
    ...
    spk_list = [spk_prefix+utt2spk[utt] for utt in utt_list]
    uttembedding_list = [kaldiio.load_mat(utt2embedding[utt]).tolist() for utt in utt_list]
    spkembedding_list = [kaldiio.load_mat(spk2embedding[utt2spk[utt]]).tolist() for utt in utt_list]
    speech_token_list = [kaldiio.load_mat(utt2speech_token[utt]).tolist() for utt in utt_list]
    if args.dpo:
        reject_speech_token_list = [kaldiio.load_mat(utt2reject_speech_token[utt]).tolist() for utt in utt_list]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--des_dir',
                        type=str)
    parser.add_argument('--spk_prefix',
                        type=str,
                        default='')
    parser.add_argument('--token_name',
                        type=str,
                        default='speech_token')
    ...
    with open('{}/embedding.scp'.format(args.src_dir)) as f:
        for l in f:
            l = l.replace('\n', '').split()
            # utt2embedding[l[0]] = kaldiio.load_mat(l[1]).tolist()
            utt2embedding[l[0]] = l[1]
    with open('{}/spk_embedding.scp'.format(args.src_dir)) as f:
        for l in f:
            l = l.replace('\n', '').split()
            # spk2embedding[l[0]] = kaldiio.load_mat(l[1]).tolist()
            spk2embedding[l[0]] = l[1]
    with open('{}/{}.scp'.format(args.src_dir, args.token_name)) as f:
        for l in f:
            l = l.replace('\n', '').split()
            # utt2speech_token[l[0]] = kaldiio.load_mat(l[1]).tolist()
            utt2speech_token[l[0]] = l[1]
    if args.dpo:
        utt2reject_speech_token = {}
        with open('{}_reject/{}.scp'.format(args.src_dir, args.token_name)) as f:
            for l in f:
                l = l.replace('\n', '').split()
                utt2reject_speech_token[l[0]] = l[1]
    ...
    for i, j in enumerate(range(0, len(utts), args.num_utts_per_parquet)):
        ...
        pool.apply_async(job, (utts[j: j + args.num_utts_per_parquet], parquet_file, utt2parquet_file, spk2parquet_file, args.spk_prefix))
    pool.close()
    pool.join()
